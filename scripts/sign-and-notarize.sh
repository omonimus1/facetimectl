...
APP_NAME="facetimectl"
CODESIGN_IDENTITY=${CODESIGN_IDENTITY:-"Developer ID Application: Peter Steinberger (Y5PE65HELJ)"}
ENTITLEMENTS="${ROOT}/Resources/facetimectl.entitlements"
OUTPUT_DIR=${OUTPUT_DIR:-/tmp}
ZIP_PATH="${OUTPUT_DIR}/facetimectl-macos.zip"
...
for ARCH in "${ARCH_LIST[@]}"; do
  swift build -c release --product facetimectl --arch "$ARCH"
done

BINARIES=()
for ARCH in "${ARCH_LIST[@]}"; do
  BINARIES+=("$ROOT/.build/${ARCH}-apple-macosx/release/facetimectl")
done

lipo -create "${BINARIES[@]}" -output "$DIST_DIR/facetimectl"

if [[ -f "$ENTITLEMENTS" ]]; then
  codesign --force --timestamp --options runtime --sign "$CODESIGN_IDENTITY" \
    --entitlements "$ENTITLEMENTS" \
    "$DIST_DIR/facetimectl"
else
  codesign --force --timestamp --options runtime --sign "$CODESIGN_IDENTITY" \
    "$DIST_DIR/facetimectl"
fi
...
codesign --verify --strict --verbose=4 "$DIST_DIR/facetimectl"
if ! spctl -a -t exec -vv "$DIST_DIR/facetimectl"; then
